...
// Helper: đảm bảo user đã có cart
...
exports.addItem = async (req, res) => {
    try {
        ...
        if (!book_id || Number.isNaN(qty) || qty <= 0) {
            return res.status(400).json({ message: 'book_id và quantity > 0 là bắt buộc' });
        }
    ...
    try {
        ...
        if (bookIds !== undefined) {
            if (!Array.isArray(bookIds) || bookIds.length === 0) {
                return res.status(400).json({ message: 'bookIds phải là mảng và không được trống' });
            }
        ...
        if (!items.length) {
            return res.status(400).json({ message: 'Giỏ hàng trống hoặc không có sách được chọn' });
        }
        ...
        if (invalidStock) {
            return res.status(400).json({ message: `Vượt quá tồn kho cho book_id ${invalidStock.book_id}` });
        }

        const subtotal = items.reduce((sum, item) => sum + Number(item.price || 0) * item.quantity, 0);
        const original_total = items.reduce((sum, item) => sum + Number(item.original_price || 0) * item.quantity, 0);
