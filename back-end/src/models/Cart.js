...
class Cart {
    ...
    // Lấy item kèm giá để tính tổng (có thể lọc theo danh sách bookId)
    ...
    // Thêm mới hoặc tăng số lượng (upsert)
    ...
    static async removeItem(cartId, bookId) {
        await db.query('DELETE FROM cart_items WHERE cart_id = ? AND book_id = ?', [cartId, bookId]);
    }
}
